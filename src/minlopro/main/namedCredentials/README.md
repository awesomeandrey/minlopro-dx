...
| `SalesforceRestApi` | _Named Credential (Legacy)_ | Used together with `Custom_Salesforce_Auth_Provider` custom auth provider (AP). The AP configuration leverages [Apex expressions](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_callouts_named_credentials.htm) to fetch secrets from `Minlopro` external credential and invoke token callout through `MinloproLogin` named credential endpoint. |
